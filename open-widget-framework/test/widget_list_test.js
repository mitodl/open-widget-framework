...
describe('<WidgetList />', () => {
  ...
  it('makePassThroughProps constructs an appropriate set of passThroughProps', () => {
    ...
    expect(moveWidgetSpy.withArgs(dummyWidgetId, dummyPosition).callCount).to.equal(1)
    ...
    expect(renderEditWidgetFormSpy.withArgs(dummyWidgetId).callCount).to.equal(1)
  ...
  it('makePassThroughProps constructs takes a widgetId and propagates it to the props', () => {
    ...
    const passThroughProps = instance.makePassThroughProps(dummyWidgetInstances[0])
    ...
    expect(renderWidgetSpy.withArgs(dummyWidgetInstances[0]).callCount).to.equal(1)
  ...
  it('renderListBody runs render widget once for each widget in widget instances', () => {
    ...
    dummyWidgetInstances.map(instance => {
      expect(renderWidgetSpy.withArgs(instance, dummyProps.listWrapperProps).callCount).to.equal(1)
    })
  })
  ...
  it('renderEditWidgetFormBody renders an EditWidgetForm', () => {
    ...
    const formWrap = mount(instance.renderEditWidgetFormBody(dummyWidgetId, dummyProps.formWrapperProps))
    expect(formWrap.exists('EditWidgetForm')).to.equal(true)
    expect(formWrap.find(EditWidgetForm).props()).to.include(dummyProps.formWrapperProps)
    expect(formWrap.find(EditWidgetForm).props()).to.include(instance.makeFormProps())
    expect(formWrap.find(EditWidgetForm).prop('widgetId')).to.equal(dummyWidgetId)
  })
  ...
  it('renderNewWidgetFormBody renders a NewWidgetForm', () => {
    ...
    const formWrap = mount(instance.renderNewWidgetFormBody(dummyProps.formWrapperProps))
    expect(formWrap.exists('NewWidgetForm')).to.equal(true)
    expect(formWrap.find(NewWidgetForm).props()).to.include(dummyProps.formWrapperProps)
    expect(formWrap.find(NewWidgetForm).props()).to.include(instance.makeFormProps())
  })
})
