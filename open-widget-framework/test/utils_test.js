...
import { apiPath, makeOptionsFromList, makeOptionsFromObject } from '../src/utils'
...
describe('apiPath', () => {
  ...
  it('returns the get_configurations url properly', () => {
    expect(apiPath('get_configurations')).to.equal('/api/v1/list/get_configurations/')
  })

  it('returns the widget_list url properly', () => {
    expect(apiPath('widget_list')).to.equal(`/api/v1/list/`)
    expect(apiPath('widget_list', dummyWidgetListId)).to.equal(`/api/v1/list/${dummyWidgetListId}/`)
  })

  it('returns the widget url properly', () => {
    expect(apiPath('widget')).to.equal(`/api/v1/widget/`)
    expect(apiPath('widget', dummyWidgetId)).to.equal(`/api/v1/widget/${dummyWidgetId}/`)
  })
})
...
describe('makeOptionsFromList', () => {
  ...
  it('returns a list of options objects that have totally matching key, label, and value to the original value', () => {
    ...
    for (let i = 0; i < options.length; i++) {
      expect(options[i]).to.deep.equal({key: optionsList[i], value: optionsList[i], label: optionsList[i]})
    }
  })
})
...
describe('makeOptionsFromObject', () => {
  ...
  it('returns a list of options objects that have totally matching key, label, and value to the original value', () => {
    ...
    for (let option of options) {
      ...
      let key = optionsObject[option.value]
      expect(option).to.deep.equal({key: key, value: option.value, label: key})
    }
  })
})
